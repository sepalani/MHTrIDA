...
|61120200|PAT ID候補送信終了返答|PAT ID candidate transmission end reply|
...
|62540100|真・名前用禁止文言要求|Prohibition wording request for true / name|
|62540200|真・名前用禁止文言応答|Forbidden wording response for true / name|
|62550100|真・名前用禁止文言取得要求|Request for acquisition of prohibited words for true / name|
|62550200|真・名前用禁止文言取得応答|Prohibition word acquisition response for true / name|
|62560100|真・名前以外用禁止文言要求|Prohibition wording request for anything other than true / name|
|62560200|真・名前以外用禁止文言応答|Prohibition word response for anything other than true / name|
|62570100|真・名前以外用禁止文言取得要求|Request to obtain prohibited words for anything other than true / name|
|62570200|真・名前以外用禁止文言取得応答|Prohibition word acquisition response for other than true / name|
...
|64141000|レイヤイン通知|Layer-in notification|
|64150100|レイヤアップ要求|Layer up request|
|64150200|レイヤアップ返答|Layer up reply|
|64151000|レイヤアウト返答|Layer out reply|
|64160100|レイヤ予約移動確認要求|Layer reservation move confirmation request|
|64160200|レイヤ予約移動確認返答|Layer reservation move confirmation reply|
|64170100|レイヤ予約移動実行要求|Layer reserved move execution request|
|64170200|レイヤ予約移動実行返答|Layer reservation move execution reply|
...
|64630100|レイヤ同期ユーザリスト要求|Layer sync user list request|
|64630200|レイヤ同期ユーザリスト返答|Layer sync user list reply|
|64640100|レイヤユーザリスト数要求|Layer user list count request|
...
|64741000|レイヤ相手指定チャット通知 (通知のみ)|Layer partner specified chat notification (notification only)|
|64741000|レイヤ相手指定チャット送信 (通知のみ)|Layer specified chat transmission (notification only)|
|64751000|レイヤユーザ用バイナリ通知 (相手指定)|Binary notification for layer users (specify the other party)|
|64751000|レイヤユーザ用バイナリ送信 (相手指定)|Binary transmission for layer users (specify the other party)|
...
|64900100|レイヤ検索詳細数要求|Layer search detail request|
|64900200|レイヤ検索詳細数返答|Layer search details number reply|
|64910100|レイヤ検索詳細データ要求|Layer search detailed data request|
|64910200|レイヤ検索詳細データ返答|Layer search detailed data reply|
|64920100|レイヤ検索詳細終了要求|Layer search details end request|
|64920200|レイヤ検索詳細終了返答|Layer search details end reply|
...
|65031000|サークルイン通知|Circle-in notification|
...
|65110100|マッチングオプション取得要求|Matching option acquisition request|
|65110200|マッチングオプション取得返答|Matching option acquisition reply|
...
|65280100|サークル検索数要求|Circle search number request|
|65280200|サークル検索数返答|Circle search number reply|
|65290100|サークル検索要求|Circle search request|
|65290200|サークル検索返答|Circle search reply|
...
|65711000|サークルバイナリ通知 (相手指定)|Circle binary notification (specified by the other party)|
|65711000|サークルバイナリ送信 (相手指定)|Circle binary transmission (specified by the other party)|
...
|66110100|相手指定チャット送信|Send specified chat|
|66110200|相手指定チャット返信|Chat reply specified by the other party|
|66111000|相手指定チャット通知|Chat notification by partner|
|66120100|相手指定バイナリ要求|Binary request|
|66120200|相手指定バイナリ返答|Binary reply specified by the other party|
|66121000|相手指定バイナリ通知|Binary notification specified by the other party|
...
|66330200|ユーザ検索数返答|Number of user searches Reply|
|66340100|ユーザ検索要求|User search request|
|66340200|ユーザ検索返答|User search response|
...
|66370100|ユーザ検索データ要求(自分)|User search data request (self)|
|66370200|ユーザ検索データ返答(自分)|User search data reply (self)|
...
|66540200|フレンドリスト返答|Friend list reply|
